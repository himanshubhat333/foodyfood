...
<html>
    ...
    <body style="margin: auto; width: 80%;">
        ...
        <form action="AddToCart" method="post">
            ...
            <input type="number" name="qty" value="${qty}" class="form-control" placeholder="Enter Qty"/>
            <br>
            
            <c:if test="${not empty invalidQty}">
                <h1 class="alert alert-danger">Invalid Quantity (1-10 allowed)</h1>
            </c:if>
            
            <input type="email" name="email" <c:if test="${not empty invalidQty}">value="${email}"</c:if> class="form-control" placeholder="Enter Email"/>
            
            <br>
            
            <c:if test="${not empty invalidEmail}">
                <h1 class="alert alert-danger">Email cannot be empty</h1>
            </c:if>
